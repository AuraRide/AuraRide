...
| 主题 token | [`default_shadcn_theme.css`](./default_shadcn_theme.css) · `src/styles/` | 颜色/圆角/间距等设计变量 |
| 构建配置 | `vite.config.ts` · `tsconfig.json` | 路径别名、插件、资源类型 |
...
1. **包管理器只用 `pnpm`**。仓库有 `pnpm-lock.yaml` 与 `pnpm-workspace.yaml`;禁止 `npm` / `yarn`,否则会破坏 lockfile。
2. **路径别名用 `@/*` → `src/*`**。新增别名必须同时改 `vite.config.ts` 和 `tsconfig.json`(两处必须镜像)。
3. **移动端优先**。所有页面经 `src/app/components/MobileOnly.tsx` 容器渲染;按移动视口设计与验证,不要做桌面端布局。
4. **优先复用,不要手搓**。UI 组件在 `src/app/components/ui/`(shadcn/ui),业务组件在 `src/app/components/`。先找现成的再考虑新建——把不变量集中,别散落重复实现。
5. **新增页面 = 两处登记**:在 `src/app/pages/` 新建组件,并在 `src/app/routes.tsx` 注册路由。两者必须一致。
6. **别动构建底座**:不要移除 `vite.config.ts` 里的 `react()` / `tailwindcss()` 插件;不要往 `assetsInclude` 加 `.css` / `.ts` / `.tsx`。
7. **不要 YOLO 猜数据形状**。静态数据在 `src/app/data/`;依赖类型定义,缺类型先补类型,别基于猜测的字段往下写。
8. **颜色契约(manifesto 衍生)**:`ColorId` 枚举与 [`docs/一切皆颜色.md`](./docs/产品/一切皆颜色.md) §1 色卡表**逐字镜像**。新增/改名/改 hex,**先改 manifesto,再改 [`src/app/lib/moodColor.ts`](./src/app/lib/moodColor.ts),再补穷尽匹配的 switch**。任何"把多模态输入压扁成颜色"的能力,写到 `src/app/lib/colorEngines/<name>.ts` 并实现 `ColorClassifier<T>` 统一签名——禁止在页面里 if/switch 临时染色。
