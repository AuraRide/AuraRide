...
description: Review a new or changed page in apps/web/src/app/pages/ against the design system (guidelines/Guidelines.md), the manifesto's "color as UX language" §5, and the mobile-first golden rule in AGENTS.md. Use when a page has been added/modified and the user wants a "is this on-brand and on-philosophy" check before merging. Read-only — produces a critique, does not edit.
...
## 2. Design system fit (`guidelines/Guidelines.md` + `apps/web/default_shadcn_theme.css`)
...
- Page registered in `apps/web/src/app/routes.tsx`? (AGENTS.md golden rule #5)
