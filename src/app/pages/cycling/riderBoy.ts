// riderBoy.ts — procedural cross-stitch "bead pixel" BOY cyclist, ported verbatim
// from boy.jsx (Claude Design handoff). Its grid is independent of the scene tile
// grid (key = x*1000 + y). Exposes build / personMask / render + cadence ratios.

export type RiderMap = Map<number, string>;
...
export interface RiderArt {
  ...
  CRANK_REV: number;
  WHEEL_REV: number;
  build: (map: RiderMap, crankAngle: number, wheelAngle: number) => void;
  personMask: (crankAngle: number) => Set<number>;
  render: (ctx: CanvasRenderingContext2D, map: RiderMap) => void;
}
...
// ── palette (boy: blonde-brown spikes, blue tee) ───────────────────
const COL = {
  out: "#46301f",
  hairD: "#7a5320",
  hairM: "#ad7c34",
  ...
  skinD: "#e0b791",
  blush: "#d98f78",
  shirt: "#6f9ec2", // blue tee
  shirtD: "#577f9f",
  shirtH: "#92b9d6",
  short: "#7f8a5c", // olive shorts
  shortD: "#69744a",
  sock: "#efe9da",
  shoe: "#b85f38", // brown shoe
  shoeD: "#8f4827",
  bike: "#cf7d4a", // orange frame
  bikeD: "#ad6033",
  bikeH: "#e29b6b",
  basket: "#9aa67e",
  basketD: "#7d8a60",
...
function drawLeg(map: RiderMap, angle: number, far: boolean) {
  ...
  thick(map, hx, hy, knee[0], knee[1], 4.2, thighC); // thigh (shorts)
  thick(map, knee[0], knee[1], px, py - 1, 3, shinC); // shin (bare skin)
  disc(map, px, py - 1.6, 1.7, far ? "#dfdac9" : COL.sock); // sock
  disc(map, px + 0.6, py + 0.6, 2.1, far ? COL.shoeD : COL.shoe); // shoe
  setCell(map, px, py + 1.4, COL.bikeD); // pedal
}
...
function drawBasket(map: RiderMap) {
  ...
  [
    ...
    [73, 48],
    [76, 48],
  ].forEach(([x, y]) => setCell(map, x, y, COL.basketD));
}

function drawHair(map: RiderMap) {
  disc(map, 56, 17, 4, COL.hairD);
  disc(map, 59, 13, 7.2, COL.hairM);
  disc(map, 54, 13, 3.6, COL.hairM);
  ...
  spikes.forEach(([tx, ty]) => {
    thick(map, tx, ty, tx - 1.5, 12, 1.9, COL.hairM);
    setCell(map, tx, ty, COL.hairL);
    setCell(map, tx - 1, ty + 1, COL.hairL);
  });
  [
    [56, 9],
    [59, 8],
    ...
    [64, 10],
    [53, 14],
  ].forEach(([x, y]) => setCell(map, x, y, COL.hairL));
}

function drawHead(map: RiderMap) {
  disc(map, 62, 17, 6.1, COL.skin);
  disc(map, 59, 20, 2.6, COL.skinD); // jaw / cheek shadow
  thinLine(map, 58, 12, 66, 12.5, COL.hairM); // hairline fringe
  setCell(map, 67, 13, COL.hairM);
  setCell(map, 65, 15, COL.out);
  setCell(map, 66, 15, COL.out); // eyebrow
  setCell(map, 65, 17, COL.out); // eye
  setCell(map, 68, 18, COL.skinD);
  setCell(map, 68, 19, COL.skinD); // nose bridge
  setCell(map, 64, 21, COL.out); // mouth
  disc(map, 64, 19.5, 1.1, COL.blush); // cheek blush
}
...
function drawBody(map: RiderMap) {
  ...
  disc(map, 61, 37, 1.9, COL.skin); // hand on grip
}

function drawFrame(map: RiderMap) {
  thick(map, RC[0], RC[1], CR[0], CR[1], 2.4, COL.bike); // chain stay
  thick(map, CR[0], CR[1], 41, 47, 2.4, COL.bike); // seat tube
  thick(map, CR[0], CR[1], 57, 41, 2.4, COL.bike); // down tube
  thick(map, 42, 47, 57, 41, 2.2, COL.bike); // top tube
  thick(map, 57, 41, FC[0], FC[1], 2.2, COL.bike); // fork
  thick(map, 57, 41, 59, 35, 2.0, COL.bike); // stem
  thick(map, 58, 35, 63, 36, 1.8, COL.bike); // handlebar
  thinLine(map, 28, 57, 43, 58, COL.bikeH);
  thinLine(map, 46, 58, 55, 42, COL.bikeH);
  disc(map, CR[0], CR[1], 2, COL.bikeD); // crank hub
  thick(map, 35, 47, 45, 47, 1.8, COL.rim); // saddle
...
// person-only cells (hair, head, body, legs) — for outlining just the rider
...
export const BoyArt: RiderArt = { GW, GH, CS, build, render, personMask, CRANK_REV, WHEEL_REV };
